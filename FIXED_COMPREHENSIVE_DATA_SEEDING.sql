-- FIXED COMPREHENSIVE SUPABASE DATA SEEDING SCRIPT
-- This script creates ALL missing tables and data with correct column names
...
-- Create settings table with correct structure (check existing first)
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'settings' AND table_schema = 'public') THEN
        ...
        CREATE INDEX idx_settings_name ON public.settings(name);
        CREATE INDEX idx_settings_company_id ON public.settings(company_id);
        
    ELSE
        -- Table exists, check if it has the right columns
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'settings' AND column_name = 'name' AND table_schema = 'public') THEN
            -- Add missing name column
            ALTER TABLE public.settings ADD COLUMN name character varying;
        END IF;
...
-- ==============================================
-- 8. CREATE REQUIRED SETTINGS DATA
-- ==============================================
...
BEGIN
    ...
    -- Global settings (no company_id)
    INSERT INTO public.settings (name, value, type, description, created_at, updated_at) VALUES
    ...
    INSERT INTO public.settings (company_id, name, value, type, description, created_at, updated_at) VALUES
...
SELECT 
    'COMPREHENSIVE DATA SEEDING COMPLETED!' as status,
