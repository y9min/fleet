...
class DriversApi extends Controller {
	...
	public function reject_ride_request(Request $request) {
		...
		if ($request->get('book_type') == 1) {
			...
			if ($count == 0) {
				$test = $this->reject_ride_notification($booking->customer_id);
				// dd($test);
			}
...

    {
        ...
        $data['data'] = array(
           ...
           'key' => "driver_unavailable_notification",

           'message' => 'Data Received.',

           'title' => "No Drivers Available. Please, Try Again Later!",
	...
	public function old_accept_ride_request(Request $request) //changed 15-09-2023
	{
		$booking = Bookings::find($request->get('booking_id'));
		...
		if ($booking == null) {
			...
			if ($u->getMeta('vehicle_id') != null) {
				$vehicles = $u->getMeta('vehicle_id');
				// dd($vehicles);
				$xx = array();
				foreach ($vehicles as $vehicle) {
					$v = VehicleModel::find($vehicle);
					if ($v->in_service == 1 && $v->type_id == $booking->getMeta('vehicle_typeid')) {
						$xx[] = $v->id;
					}
				}
				if (count($xx) >= 1) {
					// dd('if test');
					$booking->vehicle_id = $xx[0];
	...
	// public function old_accept_ride_request(Request $request)
	// {
	//     $booking = Bookings::find($request->get('booking_id'));
	//     if ($booking == null) {
	//         $data['success'] = 0;
	...
	//         // $to_date = "2023-09-15 18:37:13";
	//         // $journey_date = $booking->getMeta('journey_date');
	//         // $journey_time = $booking->getMeta('journey_time');
	//         // $from_date = date('Y-m-d H:i:s', strtotime("$journey_date $journey_time"));
	//         $vehicles = VehicleModel::whereMeta('assign_driver_id', $user_id)->get();
	//         // dump($vehicles);
	//         if(count($vehicles)>=1)
	//         {
	//             foreach($vehicles as $vehicle)
	...
	public function test_notification() {
		$id = 423;
		$booking = Bookings::find($id);
		// $sendnotification($data,$fcm_id,android_version)
		// $data['success'] = 1;
		...
		$vehicles = VehicleModel::whereIn('id', ['46', '47', '48'])->get()->pluck('id')->toArray();
		//  $data_encode =  json_encode($data);
		//  $data_decode =  json_encode($data_encode,true);
		// dd($data_decode);
		foreach ($drivers as $d) {
			// echo $d->id.'---';
			// if (in_array($d->vehicle_id, $vehicles)) {
			//     echo"<br>fcm->".$d->getMeta('fcm_id');
		...
		// $push->setMessage($data)
		//     ->setApiKey(env('server_key'))
		//     ->setDevicesToken('e9rrlpa5SVmBk9w2ygSK50:APA91bEIzaAmFFt0CbV4JHPSnp-TElxNPEJE2Sjc_VHOgTe7aBKxk-tpwPO62FhG00f_hJ_ovPLl0sTSZzyEZoNND61cWkZId9css7SZmFySyJ0ZmhFM9hI4GTILZkDeKmVkp4qVeGmy');
		//     $push = $push->send();
		//     $feedback = $push->getFeedback();
		//     if ($feedback->success == 1) {
		//         $success = 1;
	...
	public function test_notification1() {
		...
		$push->setMessage($data)
			->setApiKey(env('server_key'))
			->setDevicesToken('dF2dgDoPQjaWbZk0_UdgR1:APA91bGLPoR6uS_9uQh1Ny6JKxv4zTBDjy1MTgtdpxFsOIcT24uqaBY3AwFiN4nU-3On3XkIzufNcWXcxI2YfWec4DmGQBOs5ignfwIOhuUJ43T5TPqHne7p5yYE7VLS4QEDfFClxuS8');
		$push = $push->send();
		//dd($push);
		$feedback = $push->getFeedback();
		if ($feedback->success == 1) {
	...
	// private function isVehicleAvailable($vehicle, $from_date, $to_date)
	// {
	//     $vehicleInterval = Hyvikk::get('vehicle_interval').' MINUTE';
	//     $bookings = Bookings::where('deleted_at', null)
	//         ->where('cancellation', 0)
	//         ->where(function ($query) use ($from_date, $to_date, $vehicleInterval) {
	//             $query->whereBetween('dropoff', [$from_date, $to_date])
	//                 ->orWhereBetween('pickup', [$from_date, $to_date])
	//                 ->orWhere(function ($query) use ($from_date, $to_date, $vehicleInterval) {
	//                     $query->whereRaw("DATE_ADD(dropoff, INTERVAL $vehicleInterval) >= ?", [$from_date])
	//                         ->whereRaw("DATE_SUB(pickup, INTERVAL $vehicleInterval) <= ?", [$to_date]);
	//                 });
	//         })
	//         ->where('vehicle_id', $vehicle->id)
	//         ->count();
